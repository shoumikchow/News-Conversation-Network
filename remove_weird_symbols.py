import csv
with open("/home/shoumik/Documents/Kolpokoushol/Scraped datas/new_scraped.csv","r") as file:
    reader=csv.reader(file)
    for row in reader:
        oid = row[0]
        is_negative = row[1]
        news_crawled_date = row[2]
        news_headline = row[3].replace("‘", "").replace("’","").replace("“","\"").replace("”","\"")
        ...
        news_ner_tags__locations = row[11].replace("‘", "").replace("’","").replace("“","\"").replace("”","\"")
        news_ner_tags__locations_unique = row[12].replace("‘", "").replace("’","").replace("“","\"").replace("”","\"")
        news_ner_tags__moneys = row[13]
        news_ner_tags__moneys_unique = row[14]
        news_ner_tags__organizations = row[15].replace("‘", "").replace("’","").replace("“","\"").replace("”","\"")
        news_ner_tags__organizations_unique = row[16].replace("‘", "").replace("’","").replace("“","\"").replace("”","\"")
        news_ner_tags__percent = row[17]
        news_ner_tags__percent_unique = row[18]
        news_ner_tags__persons = row[19].replace("‘", "").replace("’","").replace("“","\"").replace("”","\"")
        news_ner_tags__persons_unique = row[20].replace("‘", "").replace("’","").replace("“","\"").replace("”","\"")
        ...
        news_text = row[26].replace("‘", "'").replace("’","'").replace("“","\"").replace("”","\"")
        news_url = row[27]
        ...
        f = csv.writer(open("/home/shoumik/Documents/Kolpokoushol/Scraped datas/new_scraped_without_weird_symbols.csv","a"))
