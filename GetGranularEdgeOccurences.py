...
for i in range(len(eventsTimeArray)):
    ...
    for x in numberOfTupleOccurencesWithTimestamp:
        ...
        if endDate in x[0][2]:
            break   
    """if (startDate[:4] == "2008" and startDate[5:7] == "12") or (startDate[:4] == "2009" and startDate[5:7] == "01"):"""

"""    print(startDate)
    print(sorted(Counter(keywordsInTimeRange).iteritems(), key=lambda x: x[1], reverse = True))
    print(endDate)"""
